...
def mdReq(app: contractApp):
    ...
    for contract in range(len(FUTCONTRACT)):
        ...
        if nextConId < len(FUTCONTRACT):
            ...
            leg1= ComboLeg()
            leg1.conId = FUTCONTRACT[currentConId].conId
            leg1.ratio = 1
            leg1.action = "SELL"
            leg1.exchange = "NYMEX"

            leg2= ComboLeg()
            leg2.conId = FUTCONTRACT[nextConId].conId
            leg2.ratio = 1
            leg2.action = "BUY"
            leg2.exchange = "NYMEX"
            ...
            app.reqMktData(requestId,mycontract,"",False,False,[])
...
def main():
    ...
    app.reqContractDetails(reqId=orderId, contract=mycontract)
    while app.contractsBuilt == False:
        pass
    mdReq(app)
